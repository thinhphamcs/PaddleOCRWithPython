...
model = YOLO(os.getenv("MODEL_OUTPUT_PATH"))
...
def display_results(items):
    """Displaying results, just raw data in tables."""
    metadata = [
        it for it in items if it["label"] in ["Account_Header", "Account_Summary"]
    ]
    if metadata:
        summary_rows = []
        for it in metadata:
    ...
    if transactions:
        ...
        for trans_block in transactions:
            lines = [
                line.strip()
                for line in trans_block["content"].split("\n")
                if line.strip()
            ]
            all_lines.extend(lines)
        if all_lines:
            st.dataframe(
                pd.DataFrame(all_lines, columns=["Transactions"]),
...
def main():
    """UI function with css and labeling."""
    ...
    uploaded_file = st.file_uploader(
        "Upload", type=["pdf", "png", "jpg", "tiff"], label_visibility="collapsed"
    )
    if uploaded_file:
